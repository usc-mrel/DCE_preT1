...
% Output Log
if verboseI
    fprintf('%10s %10s %15s %15s %15s %15s\n','Iteration','FunEvals','Step Length','Function Val','Opt Cond','Grad');
end
...
for i = 1:maxIter
    ...
    if isfield(opt, 'plot')
        if opt.plot == 1
            P = reshape(x, [opt.size(1) opt.size(2) 2]);
            ...
            title(['T1 and M0 at iter #' num2str(i)]);
            colorbar;
            axis off;
        end
    end
